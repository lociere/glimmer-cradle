...
`hosts/unity-avatar-host/` 是微光摇篮当前的 Unity/Cubism Avatar Host。Avatar 属于 Character 本体；本目录只承载 Host Adapter、具体模型驱动、渲染后端和进程资源，不定义人物身份，也不拥有 Desktop Surface。纯领域模型由 `core/avatar/` 拥有，跨进程 DTO 由 `contracts/` 生成。
