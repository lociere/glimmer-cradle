...
export class LifeClockManager {
  ...
  public start(): void {
    if (this._isRunning) {
      this.logger.warn("生命时钟已在运行中，跳过重复启动");
      return;
    }

    this.logger.info("生命时钟启动");
    this._isRunning = true;
    this._lastOrganismAttentionMode = null;
    this._lastFocusedChannelsKey = "";

    if (!this._heartbeatEnabled) {
      this.logger.info("Cognition 活性探测未启用，生命时钟不启动心跳循环");
      return;
    }
  ...
  private _updateOrganismAttention(): void {
    ...
    void this.eventBus.publish(
      new OrganismAttentionChangedEvent(
        ...
        this.observability.createTraceContext()
      )
    );
  }
  ...
  public stop(): void {
    ...
    if (this._stateSyncHandler) {
      this.eventBus.unsubscribe('StateSyncEvent', this._stateSyncHandler);
  ...
  public registerSourcePolicies(policies: Record<string, SourceAttentionPolicy>): void {
    ...
    this.logger.info("来源注意力策略已更新", { policies: this._sourceFocusPolicies });
  }
