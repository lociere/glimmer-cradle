...
export class EventBus {
  ...
  public async publish<T extends DomainEvent>(event: T): Promise<void> {
    ...
    const failures = (await Promise.all(handlers.map((entry) => entry.fn(event).catch((error) => {
      DeadLetterQueue.instance.enqueue('', eventType, event, error as Error, { owner: 'kernel', sourcePath: 'core/kernel/src/adapters/events/event-bus.ts', failurePhase: 'dispatch', retryPolicy: 'owner-confirmed', replayCommand: 'python core/kernel/tools/dlq.py replay kernel:<id> --confirm --dispatcher kernel.event-bus.v1' });
      return error;
    })))).filter(Boolean);
