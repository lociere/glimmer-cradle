...
export class ExtensionRuntimeRegistry {
  ...
  public constructor(
    private readonly availabilityContext: SkillAvailabilityContext,
  ...
  public registerManifest(manifest: ExtensionManifestRuntimeInput): ExtensionRuntimeProjection {
    ...
    const graph = buildCapabilityGraph(manifest, contributionPoints, now, this.availabilityContext, this.skillPolicy);
...
function toCapabilityGraphNode(
  ...
  const unavailableInProduct = !skillPolicy.isContributionAvailable(declaration.requirements, availabilityContext);
