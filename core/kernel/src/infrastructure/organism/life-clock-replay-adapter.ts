...
export function createLifeClockReplayAdapter(effect: (event: any) => Promise<void>) {
  return {
    ...
    deliverOrReadAck: async (request: ReplayDeliveryRequest): Promise<ReplayDeliveryAck> => {
      ...
      if (existing) return validate(existing, request, target);
      ...
      try { await writeFile(target, `${JSON.stringify(ack)}\n`, { flag: 'wx', mode: 0o600 }); } catch (error) { if ((error as NodeJS.ErrnoException).code !== 'EEXIST') throw error; return validate(await readAck(target), request, target); }
      return ack;
    },
  };
}
