...
namespace GlimmerCradle.Avatar
{
    ...
    public sealed class AvatarBehaviorProfile
    {
        ...
        public static AvatarBehaviorProfile FromDocument(AvatarBehaviorDocument document)
        {
            ...
            profile.gaze.responseTimeSeconds = Clamp(profile.gaze.responseTimeSeconds, 0.04f, 1.5f);
            profile.gaze.targetRange = Clamp(profile.gaze.targetRange, 0.1f, 4f);
            profile.gaze.pointerAttention ??= new AvatarPointerAttentionProfile();
            profile.gaze.pointerAttention.movementThreshold = Clamp(profile.gaze.pointerAttention.movementThreshold, 0.001f, 1f);
            profile.gaze.pointerAttention.holdSeconds = Clamp(profile.gaze.pointerAttention.holdSeconds, 0f, 10f);
            profile.gaze.pointerAttention.releaseSeconds = Clamp(profile.gaze.pointerAttention.releaseSeconds, 0.04f, 3f);
            profile.gaze.pointerAttention.deadZone = Clamp(profile.gaze.pointerAttention.deadZone, 0f, 0.5f);
            ...
            foreach (var idle in profile.baseline.posture.idle)
            {
                if (idle != null)
                {
                    idle.periodSeconds = Clamp(idle.periodSeconds, 1.5f, 60f);
                    idle.phase = Repeat(idle.phase, 1f);
                }
            }
            return profile;
        }
