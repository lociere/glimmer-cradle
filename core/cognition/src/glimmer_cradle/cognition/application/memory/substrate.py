...
class MemorySubstrate:
    ...
    async def retrieve(self, query: str, *, actor_id: str | None = None,
        ...
        for record in self._records.values():
            ...
            if query_vector is not None and vector is not None:
                engine = self._vector_engine
                assert engine is not None
                semantic = float(engine.cosine_similarities(
    ...
    async def _refresh_vectors(self) -> None:
        if not self._vector_ready:
            ...
            return
        engine = self._vector_engine
        repository = self._vector_repository
        assert engine is not None and repository is not None
        model = engine.model_id
        self._vectors = await repository.get_vectors("memory", model)
        missing = [record for key, record in self._records.items() if key not in self._vectors]
        if not missing:
            return
        vectors = await engine.encode(
