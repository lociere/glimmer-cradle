...
4. 产品边界：`docs/reference/product-compositions.md` 和对应 Current/Implementation。
5. 当前承诺：涉及未完成优化时读取 `docs/roadmap/now.md` 与目标 milestone。
6. 只读取目标路由、页面、feature、shared UI、样式和测试；不要用 History、外部产品或外部 Skill 推断当前事实。
...
4. 重大 UI 改造在编码前固定相同信息架构、内容和功能，产出至少三个非换色式方向；每个方向覆盖对话、系统概览、设置的宽屏与窄屏。等待用户选择或明确混合元素后，才固化当前 token 与组件语言。
...
- Control Center 与 Personal Server 可共享质量和 token 框架，不共享 Product Host、设备能力或整页装配；Presence 不复制完整管理工作台。
