...
- 总控维护可追溯控制卡并消费结构化结果，执行者直接读取事实源。跨会话、跨 owner、高风险或固定 candidate 需要交接时，必须从控制卡形成 canonical 状态/证据交接包，区分可直接消费的已确认事实、必须独立核验的固定状态、可复用验证证据和仅在触发器出现时扩读的未知项；审查独立性是独立形成结论，不是默认从零重建 inventory。低风险单执行者任务不因此增加重型控制卡或独立审查；详细机制见 `common/会话与任务编排.md`。
...
- 新会话首条提示必须声明 `session_role`、parent controller、目标、禁止项、独占资源及 owner、授权动作、状态/证据交接包、读取预算与扩读触发器、上下文传递策略、停止条件和回报契约；已有 active owner 时，新总控或执行者不得碰同一工作树或环境。
...
- 跨语言/跨进程契约由 Contract Spine 唯一拥有：可调用 Service 修改 `contracts/proto/`，可独立存储、编辑和校验的 Document 修改 `contracts/json-schema/`，统一运行 `pnpm contracts:generate` / `pnpm contracts:verify`。公开 Extension 契约由 `packages/extension-sdk` 暴露，owner-local model/projection 不得反向成为第二事实源；禁止手写 generated DTO、恢复已删除的 `protocol/` 或创建兼容壳。
