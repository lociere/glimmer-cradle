...
- 正确性、证据完整、决策可追溯与资源成本成比例同为约束。效率不是少会话、少文档、少审查、少测试或少沟通，而是消除上下文复制、重复读取、无效验证、状态噪声和返工，把投入留给真实风险边界。
- 总控维护紧凑且可追溯的控制卡与验证账本，只消费结构化结果和必要证据；执行者直接读取项目事实源并完成实现与分层验证。新会话取得完成任务所需的决策、owner、风险和账本，不复制可由事实源恢复的对话历史。
- 每个任务维护验证账本，至少记录代码状态或 commit、命令、结果、证据和失效条件。相同代码、依赖与环境上的成功检查可复用；只有相关改动、环境变化或明确验收门要求才能重跑。
- 验证按风险与反馈时延逐级升级：先运行受影响单测/静态检查，再在候选稳定后运行受影响包与真实风险门，最后按变更范围进入跨包、阶段、发布或生产验收。前一级失败时先定位根因，不得反复启动全仓测试、完整构建、Release 或生产部署碰运气；必要门不得因成本省略。
...
- 会话按风险边界、耦合度与上下文健康安排，而非按文件或微步骤机械拆分；同一 owner、紧密耦合且输入输出稳定的连续工作可由同一执行者持有，但每个架构 slice 仍须固定 commit/tree、验证账本和审查门。只有健康复核确认总控上下文下降、充分且无重复的交接明显更可靠，或用户明确要求时才更换总控。新增角色时使用 `建议创建执行会话`、`建议创建审查会话` 或 `建议创建发布/运维会话`；结束当前会话并换届时必须写出 `建议现在切换到新会话（更换总控会话）` 等包含角色的完整措辞，不得只说“切换会话”。
