...
Kernel Config Application Port 负责 OpenAI/DeepSeek 兼容 Provider 的模型发现。空 API Key 输入只在 Provider 标识、协议及解析后的模型发现地址均与已保存配置一致时复用 `configs/secrets/` 中的密钥；新目标、不同协议或 `clear_api_key` 均不复用旧密钥。显式输入的密钥仅用于本次测试，不写入配置或返回结果。

Base URL 只接受不含账号信息、查询参数和片段的 HTTP(S) 地址；根地址请求 `/v1/models`，已有 API 路径在去掉末尾斜线后追加 `/models`，保留网关前缀。测试拒绝 HTTP 重定向，网络请求及响应读取限制为 15 秒，响应体上限为 1 MiB；结果最多保留 1000 个唯一模型 ID，每项最多 256 字符，不返回含本次密钥的条目。

测试不修改 Provider、路由或 Secret。HTTP 错误只报告状态码，异常和上游正文不进入 UI 或审计；审计记录测试成败与模型数量。超时或失败后可重新测试，不能将测试失败当成基础控制面不就绪。
...
Cognition 的动态 endpoint、generation、registration nonce 与 capability secret 都不是用户配置；
它们只由 Kernel 为当前受监督子进程生成，secret 经匿名 bootstrap pipe 单次交付且不进入环境、
endpoint catalog、日志或持久文件。
