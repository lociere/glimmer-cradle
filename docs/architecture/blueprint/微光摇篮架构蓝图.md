...
上述是接受的完成态。当前仓库仍使用 `protocol/` 承载未迁移 runtime consumer；Avatar 已在 M12 Slice 5 candidate 中拆分为 `core/avatar/` 与 `hosts/unity-avatar-host/`，Extension Host 执行容器已进入 `hosts/extension-host/`。其余差距与迁移删除门见 [Protocol Reference](../../reference/protocol.md) 和 [M12](../../roadmap/milestones/M12-契约脊柱与跨进程服务架构重建.md)。
