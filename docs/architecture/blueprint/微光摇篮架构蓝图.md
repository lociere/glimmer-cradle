...
上述均是接受的目标形态，不是当前目录声明。当前仓库仍使用 `protocol/`，尚无目标 `hosts/`，`core/avatar/unity-host/` 仍混合 Avatar 领域与 Unity 承载，Extension Worker 仍位于 Kernel 内；差距与迁移删除门见 [Protocol Reference](../../reference/protocol.md) 和 [M12](../../roadmap/milestones/M12-契约脊柱与跨进程服务架构重建.md)。
...
当前平级领域/技术目录和 `protocol/generated/` 是迁移前事实。M12 必须迁移 imports、装配、生成、测试与打包消费者后删除旧路径；接受本蓝图不表示目标层次已经落地。
...
这是 [ADR-0014](../decisions/ADR-0014-仓库物理分层与器官模块边界.md) 对 [ADR-0005](../decisions/ADR-0005-Character-Avatar-Surface-Host分层.md) 和 [ADR-0007](../decisions/ADR-0007-UnityAvatarHost程序集边界与SDK投影.md) 相关物理/编译落点的部分替代；二者的语义分层与单向依赖继续有效。当前 `core/avatar/unity-host/` 仍混合领域与 Unity 承载，Domain/Application 仍引用 `UnityEngine`，只是迁移前事实。M12 必须完成独立 Core 构建测试、Host Adapter 映射、单向 assembly reference、Unity/Desktop/package build 和旧六 asmdef/旧目录/复制源码删除门。目标 Unity 若无法消费选定 TFM/assembly，必须停下切片并用新 ADR 修改机制，不得退回混合 Unity Domain。
