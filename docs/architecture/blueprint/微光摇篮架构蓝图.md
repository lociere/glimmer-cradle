...
| 目录 | 蓝图含义 |
|---|---|
| `contracts/` | 长期目标的契约脊柱：Protobuf Service、JSON Schema 文档契约、兼容与生成配置 |
...
`contracts/` 是 [ADR-0013](../decisions/ADR-0013-契约脊柱与跨进程服务架构.md) 接受的目标物理形态，不是当前目录事实。当前代码仍使用 `protocol/`、JSON Schema 生成投影和既有 ZMQ/stdio/WebSocket 链路；差距与迁移边界见 [Protocol Reference](../../reference/protocol.md) 和 [M12](../../roadmap/milestones/M12-契约脊柱与跨进程服务架构重建.md)。
...
1. 这个能力属于 Cognition、Kernel、Desktop、Avatar、Engine、Native、Extension、Skill Plane、Contracts 还是 Data？
2. 它是器官、能力、表面、投影还是缓存？
3. 谁是唯一事实源？
4. 哪些结构跨语言、跨进程、跨信任边界？
