...
| `packages/` | 公开 SDK、稳定跨产品库或可独立测试与版本治理的开发包，不是内部杂物抽屉 |
...
`products/` 只拥有产品组合。至少两个真实产品出现稳定复用，且候选具备独立 API、版本、测试和 owner 后，才从产品中提取 design primitives 或 surface client 到 `packages/`；不为未来可能复用而预建 UI shared package。
...
- Domain 拥有监督、权限、路由、Ingress、catalog 和 Projection 规则；
- Application 按有名称的 use case 编排，不建立无 owner 的 `application/services` 聚合；
- Ports 不 import transport、generated DTO 或具体 runtime；
- provider、persistence、transport、process、observability 和 contract 映射属于 Adapters；
- Runtime 只拥有生命周期、readiness、进程世代、start/stop/restart；
- Composition 是创建具体实现和接线的唯一根。

当前 `foundation/`、`application/services/`、`infrastructure/`、`host/` 等物理事实只由 Current/Implementation 描述。M12 必须把其中能力迁给明确 owner，并在 imports、配置、测试、构建与打包消费者归零后删除旧聚合目录和兼容导出；不能把旧目录更名为新架构外壳。
...
长期内部物理层次是 `domain/`、`application/`、`ports/`、`adapters/`、`host/`。领域模块通过本地 Application use case、明确事务边界、Port 与领域事件协作；provider、transport、persistence、observability 才是 Adapter。generated contract DTO/stub 只能停留在 contract Adapter 边缘，不进入心智 Domain/Application。
...
该领域长期物理归属是 `core/avatar/`，拥有 Domain/Application/Ports；Unity/Cubism driver、进程 bootstrap、framework 生命周期和 Host 边界 Adapter 归 `hosts/` 下的 UnityAvatarHost。当前 `core/avatar/unity-host/` 把两者混在一起，只是迁移前事实。M12 必须完成程序集、imports、composition root、生成、配置、测试、构建和打包迁移后删除旧目录与旧构建入口，不能只改目录名或保留兼容外壳。
