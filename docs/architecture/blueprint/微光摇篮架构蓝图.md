...
> 状态：与 [Architecture Baseline v2.0](./Glimmer_Cradle_Architecture_Baseline_v2.0_Frozen.md) 冲突的目标设计已被替代。
> 替代范围由 [ADR-0019](../decisions/ADR-0019-采用Architecture-Baseline-v2冻结基线.md) 固定；不冲突的产品需求继续有效。
...
工程生命周期平面同样是平台级承重面，但它不属于这五根角色运行时脊柱，也不与认知核、中枢监督树、身体、能力生态或记忆脊柱混称。它负责把源码和 owner-local 任务可靠地变成可验证制品与宿主事务，使上述器官能够持续构建、交付、安装、恢复和演化；其长期边界见 [2.5 工程生命周期平面](#25-工程生命周期平面) 与 [ADR-0015](../decisions/ADR-0015-工程自动化平面与交付生命周期分层.md)。
...
精确当前目录事实见 Current、Implementation 和 Reference；蓝图只保留长期归属。[ADR-0014](../decisions/ADR-0014-仓库物理分层与器官模块边界.md) 固定根职责、Host 归属和器官内部分层，[ADR-0013](../decisions/ADR-0013-契约脊柱与跨进程服务架构.md) 固定 Contract Spine 与跨进程 Service。
...
这套拓扑与 [ADR-0009](../decisions/ADR-0009-本地监督树与动态端点治理.md) 组合：gRPC/Connect 固定契约和调用语义，ADR-0009 固定本地监督、动态回环端点和真实 readiness。二者都不要求固定端口。
...
DDD/Hexagonal、BFF/Projection、Saga 和 control/data plane 是架构模式；Protobuf、gRPC、JSON Schema、Buf 与 Connect 是 IDL、规范、工具或 transport 生态。摇篮组合这些成熟方法以适配 TS/Python/C#、本地多进程、多个 Surface、Extension 与媒体器官，不复制任何厂商模板。技术治理、许可与防锁定规则由 [ADR-0013](../decisions/ADR-0013-契约脊柱与跨进程服务架构.md) 固定。
...
这是 [ADR-0014](../decisions/ADR-0014-仓库物理分层与器官模块边界.md) 对 [ADR-0005](../decisions/ADR-0005-Character-Avatar-Surface-Host分层.md) 和 [ADR-0007](../decisions/ADR-0007-UnityAvatarHost程序集边界与SDK投影.md) 相关物理/编译落点的部分替代；二者的语义分层与单向依赖继续有效。M12 Slice 5 candidate 已完成独立 Core 构建测试、Host Adapter 映射、单向 assembly reference 和旧六 asmdef/旧目录/复制源码删除门；后续不得退回混合 Unity Domain。
