...
> 事实依据：workspace 包、`products/`、`contracts/`、`packages/extension-sdk/`、`core/`、`engines/`、`native/`、已安装扩展目录、生成物和当前实现。
...
| 构件 | 职责 | 不承担 |
|---|---|---|
| `core/platform/` | Time、Identity、Observability、Lifecycle、LiveEvent 公共契约与生命周期协调 | 产品装配、领域事件、readiness 决策、durable replay、DLQ |
...
| `packages/extension-sdk/` | Extension 公开 API、manifest/package validator、权限与 Host edge mapping | Kernel 内部对象、产品 view model、canonical Document Schema |
...
flowchart TD
  ...
  Contracts --> Extensions[extension-sdk public edge]
...
- Service/Document 分别在 `contracts/proto/`、`contracts/json-schema/` 定义并运行 `pnpm contracts:generate` / `pnpm contracts:verify`。公开 Extension API 由 SDK 暴露；消费者不得复制 canonical 字段。
- `protocol/` 已物理删除；不得恢复 ZMQ、stdio、器官间手写 WebSocket、多根生成主线或重命名后的万能 helper 壳。
...
| 问题 | 应放位置 |
|---|---|
| “这是跨进程可调用能力 / 文档型契约” | `contracts/proto/` / `contracts/json-schema/` |
| “这是 Extension 开发者稳定消费的公开边缘” | `packages/extension-sdk/` |
