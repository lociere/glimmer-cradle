...
> 事实依据：workspace 包、`products/`、`protocol/`、M12 Slice 1 `contracts/` baseline、`core/`、`engines/`、`native/`、已安装扩展目录、生成物和当前实现。
...
| 构件 | 职责 | 不承担 |
|---|---|---|
| `protocol/` | Slice 9 前尚未迁移的 runtime JSON Schema、TS projection 与 runtime helper | Cognition/Surface/Avatar/Audio 等已迁移 Service、Python/C# projection、手写镜像 |
| `contracts/` | Contract Spine Protobuf Service、JSON Schema Document、Buf 生成物、兼容基线、三语言 round-trip 和供应链证据 | Domain/Application/Port 内部模型、UI view model、媒体字节 |
...
| `core/avatar/` + `hosts/unity-avatar-host/` | Unity 外 Avatar Core 与具体 Unity/Cubism Host；Host Adapter 消费 Contract Spine Avatar v1 | Control Center、Kernel 生命周期总控、平台配置事实源 |
...
flowchart TD
  ...
  Contracts[contracts Service and Document] --> Kernel[core/kernel]
  Contracts --> Cognition[core/cognition adapters]
  Contracts --> Engines[engine adapters]
  Protocol[remaining protocol Documents] --> Kernel
  ...
  Cognition --> Generated[Generated Contract DTO]
  Renderer --> Preload[Preload API]
  Extensions --> SDK[Extension SDK]
  Engines --> EngineProtocol[Audio gRPC and media references]
```

规则：

- 迁移期先判断契约 owner：对应切片前的现有 runtime 结构继续在 `protocol/src/schemas/` 定义并运行 `pnpm sync:contracts`；新 Service/Document 分别在 `contracts/proto/`、`contracts/json-schema/` 定义并运行 `pnpm contracts:generate` / `pnpm contracts:verify`。消费者不得复制字段。
- Cognition、Surface、Avatar 与 Audio runtime 已通过对应删除门切到 `contracts/`；`protocol/` 只保留 Slice 9 尚待迁移的 Document/helper consumer，不得恢复已删除的 ZMQ、WebSocket、stdio 或多根生成主线。
...
| 问题 | 应放位置 |
|---|---|
| “这是尚未迁移的现有 runtime 字段，两个进程都要理解” | `protocol/src/schemas/` |
| “这是新 Contract Spine 的跨进程可调用能力 / 文档型契约” | `contracts/proto/` / `contracts/json-schema/` |
