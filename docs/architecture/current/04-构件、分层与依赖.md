...
| 构件 | 职责 | 不承担 |
|---|---|---|
| `protocol/` | 尚未迁移的现有 runtime JSON Schema、生成器、TS/Python 投影、跨语言/跨进程共享模型 | 新 Contract Spine Service/Document、TS-only 便利类型、UI view model、手写镜像 |
| `contracts/` | M12 Slice 1 已落地的 Contract Spine baseline、Protobuf Service、JSON Schema Document、Buf 生成物、兼容基线、三语言最小 round-trip 和供应链证据 | 当前 runtime transport、旧 `protocol/` consumer、Domain/Application/Port 依赖 |
...
- M12 Slice 1 只落地长期 Contract Spine baseline；当前运行代码仍使用 `protocol/`。只有后续迁移切片通过对应删除门后，runtime consumer 才能切到 `contracts/`，Current 届时再更新。
