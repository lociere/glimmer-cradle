...
表中的目标是职责落点；未实施行不代表最终包名、数据路径或完整文件树已确定。
各切片实施前须在 [执行记录](../../roadmap/architecture-v2-refactor.md) 补齐具体路径和验证计划。
...
| Kernel conversation binding、Cognition Experience/Conversation stores | Conversation | 交互事实迁 canonical log，History 保持 projection；保护稳定 ID、顺序、checkpoint 与已有非交互经验 |
...
| `products/desktop/`、`products/personal-server/` | `apps/desktop/`、`apps/server/` | 启动、安装、发布和路径投影同步；源码改名不等于外部分发升级 |
