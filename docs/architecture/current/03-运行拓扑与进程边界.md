...
Kernel 是本体能力的 composition root。`tools/workspace-supervisor/` 是开发期产品组合监督者，读取 `products/desktop/product.json` 或 `products/personal-server/product.json`，并通过 Product package manifest 声明的 `product:prepare`、`product:dev`、`product:start` 入口共同持有 Kernel 与 Product Host 进程树；它不参与业务路由或 readiness 判断。Desktop preparation 可调用 Desktop owner-local `prepare-runtime`，Personal Server preparation 不经过 Desktop assets。其他进程不能绕过 Kernel 建立隐式控制关系。
...
| Cognition | Python 认知核 | 人格、情绪、记忆、推理依赖 Python AI 生态，故障应隔离 | 本代 gRPC Service 注册、配置加载、持久化/记忆基础设施、认知循环可服务 |
...
| Audio Engine | Python `engines/audio` 动态回环 gRPC | TTS/ASR 模型依赖与冷启动独立于 Kernel | 进程级令牌认证、资源 catalog、依赖、模型 warmup 和 Service health 完成或明确 degraded |
| Extension Host | 每个启用 Extension 的独立 `hosts/extension-host` Node 子进程 | 可安装生态能力需要独立依赖与故障域 | manifest 校验、Host process connected/handshake/resource prepared/ready、权限约束的 Port RPC、激活和 disposable 生命周期建立 |
...
2. **Transport**：建立 Kernel 对外和内部通信通道，例如 gRPC/WebSocket/stdio 管道。
...
Kernel 的 Cognition RPC、Surface Gateway、Avatar Host 与 Audio Engine 都使用 `127.0.0.1` 动态 gRPC 端点。`EndpointRegistry` 管理 Kernel-owned Service 的本代 owner PID、generation、purpose 和 endpoint；Audio 子进程则用一次性启动帧只向直接父进程报告端点，不写入公共 catalog。Avatar 与每个 Audio lane 都使用仅由 Kernel 注入受管进程环境的随机令牌认证，不把令牌写入 catalog、配置或日志。普通配置不再暴露这些端口；Personal Server 通过自己的受认证 WebSocket/HTTP ingress 暴露服务，不能把内部端点改成外网绑定。
...
Personal Server 的 `/healthz` 只表达 Product Host 进程和 HTTP listener 存活；`/readyz` 通过内部 Surface Gateway gRPC stream 持续消费 Kernel 的 canonical runtime catalog。只有 `kernel.ingress=ready` 且全部 blocking runtime 为 `ready` 时才返回 `200`。端点尚未发布、Surface Gateway 断连或 catalog 失去权威观测时立即撤销 ready，不能沿用最后一次成功快照。

Cognition 进程意外退出或启动失败时，Kernel 同步把 `cognition` 与 required
`kernel.ingress` 投影为 failed 并关闭 Ingress；自动重启只有在新 generation 重新完成认证注册、
知识初始化和业务 readiness 后才恢复原本已开放的 Ingress。重启失败保持 failed，不能用旧代
endpoint、旧 ready 或进程存在状态兜底。
...
- Extension Host process ready 只说明 IPC 已连接；扩展入口加载、资源准备、激活注册和 capability readiness 必须分阶段投影。注册的能力必须经过 Policy 和 Gateway 才可调用。
...
重启策略只适用于可以安全重建的 runtime。Cognition、Audio、Avatar、Extension Host 或 MCP server 重启后，Kernel 必须重新建立握手、ready 和 catalog 状态；不能复用旧 handler、旧端口或旧状态投影。
