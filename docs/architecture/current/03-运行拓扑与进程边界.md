...
Kernel 的 Cognition RPC、Surface Gateway 和 Avatar Host 都绑定 `127.0.0.1` 动态端口；其中 Surface Gateway 使用 `grpc://` Contract Spine Service，Avatar Host 的 WebSocket control 属于 M12 Slice 8。`EndpointRegistry` 将本代 owner PID、generation、purpose 和 endpoint 原子写入 `data/run/host/endpoints.json`；Desktop、Personal Server、Cognition 和 Unity 只消费本代端点。普通配置不再暴露这些端口，最后一个端点撤销或停机时删除 catalog。Personal Server 通过自己的受认证 WebSocket/HTTP ingress 暴露服务，不能把内部端点改成外网绑定。
