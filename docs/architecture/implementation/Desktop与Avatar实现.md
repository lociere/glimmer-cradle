...
Windows 受管启动先由 `products/desktop/src/main/packaged-supervisor.ts` 通过同包 `DesktopProcessTreeBridge.exe` 建立 Desktop-owned Job Object。helper 以 `CREATE_SUSPENDED` 创建 Kernel，先 `AssignProcessToJobObject` 再 resume，并通过受控 stdio JSON 返回 launch session、root PID、active process count 与 termination result；root exit 不释放 Job authority，stop/timeout 必须确认 active count 为零，否则保持 failed。缺少 helper 或协议/计数校验失败时 Desktop 首启 fail closed。`UnityAvatarHostLauncher.exe` 仍只拥有 Avatar 窗口创建、透明隔离和 Composition Surface，不是 Desktop 进程树 owner。`AvatarCompositionHost` 在调用 `Screen.SetResolution` 前再次隔离容器窗口；Unity worker 不允许在脚本接管前短暂出现在左上角。
