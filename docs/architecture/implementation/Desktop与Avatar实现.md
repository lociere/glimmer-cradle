...
形象页由 `AvatarPage` 提供大预览、模型选择、显示比例、桌面驻留和手动动作；`ControlCenter` 在 renderer store 中更新草稿，并通过 `window.desktopHost.setAvatarAppearance()` 写回 Electron main，切页、卸载和显式提交都会 flush 待保存形象状态。预览图来自 `assets/avatar/avatar-packages/*/avatar-package.json` 的 `previewImagePath`，由 `products/desktop/scripts/sync-assets.mjs` 投影到 renderer public 目录，只作为 Control Center Web 预览，不代表 Avatar 正式渲染资产。当前形象主线以 Kernel 广播的 `character_presentation_projection` 为准：`renderer/host/useDesktopHost.ts` 负责首取和订阅，再写入 `renderer/store/appStore.ts`；Control Center、Presence 和 Unity Avatar 都消费同一份身体形象投影，renderer 不再直接拼 `models.json` 或本地文件事实。形象页和 Optional Inspector 的 SDK、Avatar Package Registry、Host executable、工作目录状态优先读取 `runtime_readiness` 中 `avatar.host.reconciler.resources`，只有 Kernel 投影尚未到达时才使用 `getAvatarDiagnostics()` 作为本机路径与打开位置回退；本地 diagnostics 不是形象 ready 的事实源。动作状态以 Avatar Host 回报和重新读取结果为准，不做乐观翻转。Kernel 通过 `GLIMMER_CRADLE_AVATAR_ACTION_STATE_PATH` 把 `data/state/avatar/action-state.json` 注入 UnityAvatarHost；Host 自有 `AvatarActionScheduler` 只持久化 `activeActionIds`，并以 `AvatarHostService` 的 `avatar_action_state` 回报真实投影。Kernel 在本轮尚未收到 Avatar 状态时保持 `null`，不向 Desktop 广播伪造的空快照；读写失败进入 Avatar process log。
...
| `core/kernel/src/adapters/avatar/avatar-controller.ts` | Kernel `AvatarHostService.Connect` control owner、认证、readiness 与投影映射 |
| `Host/UnityAvatarHostBootstrap.cs` | UnityAvatarHost 启动与 wiring |
| `Adapters/AvatarContractAdapter.cs` | Contract Spine C# generated DTO 与 Core command/event 的唯一双向映射；验证 kind、唯一 payload、必填 presence 与语义枚举，不经过 JSON round-trip |
| `Adapters/AvatarGrpcTransport.cs` / `Host/AvatarProtocolClient.cs` | 固定 `Grpc.Core` 双向 stream transport 与 Host 生命周期 client；二进制帧只在 Adapter 边缘映射，typed failure 上报稳定 `error` |
...
Unity 只消费 Avatar Contract Spine projection 和模型 catalog 投影，不读取 Kernel 内部配置。项目 Assembly Definition 固化 `Core + generated Contracts -> Adapters、Core + Live2D.Cubism -> Infrastructure、Adapters + Infrastructure + Core -> Host -> Editor` 的单向引用；只有 Infrastructure 可以引用第三方 `Live2D.Cubism`。Avatar wire 由 `contracts/proto/glimmer/avatar/v1/avatar_host.proto` 权威拥有；proto3 scalar presence 与必填语义对齐，使 Adapter 能区分缺失与显式 `false/0`。Kernel 与 Unity Adapter 都直接映射 generated DTO，旧 JSON envelope、formatter/parser consumer 与 legacy `PresentationFrames.g.cs` 已删除。模型投影和 StreamingAssets 是构建/同步产物，不是手工事实源。

Kernel `AvatarHostService.Connect` 绑定动态回环 gRPC 端点，并通过 `GLIMMER_CRADLE_AVATAR_GRPC_URL` 与 `GLIMMER_CRADLE_AVATAR_AUTH_TOKEN` 注入受管 Unity Host；`avatar-host.json` 不保存端口或令牌。Kernel 使用 `@grpc/grpc-js` 承载双向 stream，Unity Adapter 使用固定 `Grpc.Core 2.46.6` managed/native runtime；generated DTO 只在双方 Adapter 边缘。缺失令牌、错误令牌、取消、断连、心跳超时和停机都会撤销 ready，旧 Kernel↔UnityAvatarHost WebSocket control consumer 与环境变量已删除。M12 Slice 7 的 Desktop main 与 `PackagedSupervisor` 则通过 `main/surface-gateway-client.ts` 连接 `control-surface` 的 `SurfaceGatewayService` gRPC，使用 catalog generation 完成 Connect 认证，并以 Stream 首帧 `runtime_readiness` 验证所有 blocking runtime；端点存在本身不能宣称 ready。timeout 或 stop 只有确认进程树退出后才能投影 `stopped`，否则保留 `failed` 诊断。开发态 `dev-electron.mjs` 等待同一目录，不保留独立固定端口逻辑。
...
Windows 受管启动先由 `products/desktop/src/main/packaged-supervisor.ts` 通过同包 `DesktopProcessTreeBridge.exe` 建立 Desktop-owned Job Object。helper 以 `CREATE_SUSPENDED` 创建 Kernel，先 `AssignProcessToJobObject` 再 resume，并通过受控 stdio JSON 返回 launch session、root PID、active process count 与 termination result；root exit 不释放 Job authority，stop/timeout 必须确认 active count 为零，否则保持 failed。缺少 helper 或协议/计数校验失败时 Desktop 首启 fail closed。Native Composition 的安装态 canonical 文件位于 `resources/components/native/composition-host/bin/Release/`：`platform_native.dll` 提供平台边界，`UnityAvatarHostLauncher.exe` 由该组件唯一拥有并负责 Avatar 窗口创建、透明隔离和 Composition Surface；launcher 不是 Desktop 进程树 owner，也不复制到 Avatar component。`AvatarCompositionHost` 在调用 `Screen.SetResolution` 前再次隔离容器窗口；Unity worker 不允许在脚本接管前短暂出现在左上角。
