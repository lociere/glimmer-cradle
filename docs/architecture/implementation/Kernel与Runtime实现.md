...
> 源码依据：`core/kernel/src/app.ts`、`main.ts`、`core/kernel/src/adapters/cognition/`、`core/lifecycle/runtime/`、`foundation/`、`domain/`、`application/`、`host/`。
...
| `core/kernel/src/app.ts` | Kernel composition root，组装配置、日志、事件总线、storage、Cognition gRPC transport、runtime 与 application service |
...
`app.ts` 是跨层具体实现的集中组装点。普通业务对象不应自行 new logger、storage、process supervisor、gRPC host/client 或 provider manager；需要替换实现时应通过 root/factory/port 完成。
