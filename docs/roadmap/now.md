...
[M12：契约脊柱与跨进程服务架构重建](./milestones/M12-契约脊柱与跨进程服务架构重建.md) 的 Slice 1 与 Slice 2 已集成：`contracts/` baseline、三语言生成/兼容链、Kernel↔Cognition v1 Service 与受 Kernel 监督的动态回环 gRPC 已落地，该边界旧 ZMQ/envelope 主线已删除。Slice 3 已在当前分支形成验证通过的 clean candidate：Kernel 物理分层、强类型 Ports、Composition/Runtime 注入、真实 production bootstrap 与 architecture gate 已收口；该 candidate 尚待独立审查和集成。Slice 4～9 尚未启动；M11 仍暂停/延期且未完成。
