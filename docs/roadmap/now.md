...
[M12：契约脊柱与跨进程服务架构重建](./milestones/M12-契约脊柱与跨进程服务架构重建.md) 的 Slice 1～8 已形成固定提交，Slice 9 legacy protocol closure 正在形成修复 candidate：旧 `protocol/` 已物理删除；独立 Document 已迁入 `contracts/json-schema/` compatibility baseline，Service/DTO consumer 已切到 Contract Spine edge。Surface Gateway 使用有限 typed Query/Command/Event DTO，Kernel 与产品 Adapter 映射 owner-local request/projection；公开 Extension SDK 只保留扩展作者与 Host Port 所需 API，系统配置、Control Center、安装和运行投影回归 Kernel/Product owner。此前同一固定 candidate 上重跑的 Personal Server production smoke 已 exit 0，DeepSeek 不再返回 402，startup/readiness、真实 conversation reply 与 clean shutdown 均通过。当前 findings 修复仍待固定提交与独立复审，不写成 M12 完成；第三方 Cubism SDK 仍只存在于 ignored 本机供应目录，不进入 Git。M11 仍暂停/延期且未完成。
