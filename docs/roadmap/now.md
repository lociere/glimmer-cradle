...
[M12：契约脊柱与跨进程服务架构重建](./milestones/M12-契约脊柱与跨进程服务架构重建.md) 的 Slice 1 与 Slice 2 已完成：`contracts/` baseline、三语言生成/兼容链、Kernel↔Cognition v1 Service 与受 Kernel 监督的动态回环 gRPC 已落地，该边界旧 ZMQ/envelope 主线已删除。Slice 3～9 尚未启动；M11 仍暂停/延期且未完成。
