...
[M12：契约脊柱与跨进程服务架构重建](./milestones/M12-契约脊柱与跨进程服务架构重建.md) 已依据 [ADR-0013](../architecture/decisions/ADR-0013-契约脊柱与跨进程服务架构.md) 接受为 `planned` 下一阶段，但尚未开始，不改变 M11 的唯一 active 状态，也不把目标架构写成当前事实。
