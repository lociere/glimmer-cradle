...
- `[x]` Extension 发布主线：`@glimmer-cradle/contracts@0.1.0` 与 `@glimmer-cradle/extension-sdk@0.1.0` 已由精确 SDK tag 带 provenance 发布到 npm；NapCat `v0.1.0` 已发布双平台 `.gcex`、Release Manifest、`SHA256SUMS` 与 provenance，默认 Registry 的 `approved/verified` stable 指针绑定该不可变 Release。SDK 与 Adapter workflow 均可对精确既有 tag 做不覆盖发行记录的幂等重建，Node 24 Artifact Actions 的上传/下载和摘要复验已在 Ubuntu 24.04 真实运行通过；主仓从公开 Registry 解析 GitHub Release 并选择 Linux `.gcex`，完成信任预览、权限确认、原子安装、拒权不破坏既有安装、同摘要幂等重装与卸载。SDK → npm → Adapter tag → Release → Registry → Linux Package Manager 的发布链已闭合。
- `[~]` NapCat 跨产品化：`0.2.0` SDK、Personal Server 与 Adapter 已发布。Kernel 仅通过通用 `ExtensionContext.activationProfile` 和需 `SECRET_READ_SELF` 的 `ctx.ports.secrets.get(key)` 提供运行形态与可选扩展 Secret；普通 config 保持扩展 Schema 拥有的非敏感对象。Adapter 以 Host 为唯一 profile owner，`EXTERNAL_PROCESS`、Windows command/resource/setting 只属于 `managed_napcat_windows`，Personal Server/Linux 的 `external_onebot` 只保留所需权限和 contribution；主仓不解释或硬编码第三方字段。`0.2.1` 的 SDK、Personal Server、Adapter 与 Registry 已发布并运行于生产，但宿主备份 owner 复核未通过，故继续以 `0.2.2` 整体状态分域架构收口；Extension Host 生产激活、external OneBot 真实 QQ E2E、生产配置/可选 Secret 和重启连续性仍待完成。
- `[~]` 生产验收：`0.2.1` 已在生产达到 ready、原子 projection、固定 Caddyfile 挂载和 Ops Bridge 可用，但备份位于 service-owned 父树的设计不能形成可信边界，因此不接受为完成。ADR-0018 规定 `/var/lib/glimmer-cradle/{host,service}` 与 `/run/glimmer-cradle/{host,service}` 对称分域，Host 事务库只消费显式 host root；新主线不探测或回退旧目录。`0.2.2` 定向契约、宿主事务沙箱、58 项产品测试与 Contract Spine 已通过；固定完整包、生产离线迁移、旧路径删除、失败候选回滚、备份恢复和长运行仍待复验。
...
- `[~]` Extension 统一事务 UI/投影已覆盖仓库/Registry/Release Manifest 预览、安装、激活、卸载、版本切换回退与浏览器本地 `.gcex` 上传；公开 Registry 到 Ubuntu 24.04 Linux Package Manager 的真实发布物安装/拒权恢复/幂等/卸载已通过。跨版本升级与回滚等待第二个获准候选；真实 Product Host 激活和配置场景仍未完成。
- `[~]` NapCat Linux `.gcex` 已正式发布并通过公开 Registry 安装；Windows 主机与 Ubuntu runner 均按 `personal-server` 产品约束完成包事务探针。Linux Extension Host、外部 OneBot 私聊/群聊/记忆链路和重启连续性验收未完成。
