...
- `[~]` NapCat 跨产品化：`0.2.0` 固定候选由 Kernel 通过通用 `ExtensionContext.activationProfile` 传递 Host 已解析的运行形态，并以需 `SECRET_READ_SELF` 的 `ctx.ports.secrets.get(key)` 提供可选扩展自有 Secret；普通 config 保持扩展 Schema 拥有的非敏感对象，不需要 Secret 的扩展不声明权限。Adapter 删除普通配置中的第二 profile owner，并将 `EXTERNAL_PROCESS`、Windows command/resource/setting 限定到 `managed_napcat_windows`；Personal Server/Linux 的 `external_onebot` 只保留所需权限和 contribution。主仓不解释或硬编码第三方字段。根 `check:pr`、Contract gates、SDK 干净消费者发布校验，以及 Adapter 17 项测试与双平台 `release:prepare` 已通过。发布、Extension Host 生产激活、external OneBot 的真实 QQ 场景 E2E、生产配置/Secret 和重启连续性仍待完成。
