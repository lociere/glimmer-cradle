...
- `[~]` NapCat 跨产品化：`0.2.0` SDK、Personal Server 与 Adapter 已发布。Kernel 仅通过通用 `ExtensionContext.activationProfile` 和需 `SECRET_READ_SELF` 的 `ctx.ports.secrets.get(key)` 提供运行形态与可选扩展 Secret；普通 config 保持扩展 Schema 拥有的非敏感对象。Adapter 以 Host 为唯一 profile owner，`EXTERNAL_PROCESS`、Windows command/resource/setting 只属于 `managed_napcat_windows`，Personal Server/Linux 的 `external_onebot` 只保留所需权限和 contribution；主仓不解释或硬编码第三方字段。`0.2.1` 架构修订候选已通过根 `check:pr` 与真实完整包事务验收，发布、Extension Host 生产激活、external OneBot 的真实 QQ 场景 E2E、生产配置/Secret 和重启连续性仍待完成。
- `[~]` 生产验收：生产 `0.2.0` 更新暴露宿主 run root 与服务 UID 混用、候选 Caddyfile 提前污染 canonical env 两项架构缺陷；自动回滚未能自行恢复入口，但已用未受损事务备份将 `0.1.8` 服务恢复 ready。`0.2.1` 删除该共享 owner 模型，拆分 host-owner/service 与持久数据/备份域，候选 projection 仅在 readiness 后原子提交；本地真实完整包已通过离线安装、重复安装、备份恢复、不健康候选回滚、数据和 deployment env 连续性、Caddyfile 挂载及 stop 回收。正式发布和同一生产机复验仍待完成。
