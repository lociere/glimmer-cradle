...
默认链路使用 GitHub Release、GHCR 和 Docker Hub。它不修改系统 DNS，也不自动注入未知镜像站。阿里云区域访问这些站点不稳定时，应把同一组已校验发布物同步到自己的 OSS，把 OCI 镜像同步到自己的 ACR，然后显式覆盖来源：

```bash
curl -fsSL https://<bucket>.<region>.aliyuncs.com/glimmer-cradle/glimmer-cradle-installer.sh | \
  sudo env \
    GLIMMER_CRADLE_DOWNLOAD_BASE=https://<bucket>.<region>.aliyuncs.com/glimmer-cradle/0.1.0 \
    GLIMMER_CRADLE_CANDIDATE_IMAGE=registry.<region>.aliyuncs.com/<namespace>/glimmer-cradle-personal-server:v0.1.0@sha256:<digest> \
    GLIMMER_CRADLE_CADDY_IMAGE=registry.<region>.aliyuncs.com/<namespace>/caddy:2.10.2-alpine \
    bash
```
...
- `GLIMMER_CRADLE_CANDIDATE_IMAGE` 指向相同版本的 ACR 应用镜像；
- `GLIMMER_CRADLE_CADDY_IMAGE` 仅在已同步并信任对应基础镜像时设置。
