...
> 状态：与 [Architecture Baseline v2.0](../../architecture/blueprint/Glimmer_Cradle_Architecture_Baseline_v2.0_Frozen.md) 冲突的目标设计已被替代。
> 替代范围由 [ADR-0019](../../architecture/decisions/ADR-0019-采用Architecture-Baseline-v2冻结基线.md) 固定；不冲突的产品需求继续有效。
...
| [精确目标物理拓扑](目标物理拓扑.md) | 蓝图完全落地后的 Repository、Build、Distribution、Installed Host 与 Runtime Data 路径是什么 |
| [1. 架构宪法](#1-架构宪法) | Glimmer Cradle 与默认角色 Selrena 的关系是什么，哪些原则不可违背 |
| [2. 项目模块定位总图](#2-项目模块定位总图) | Contracts、Kernel、Cognition、Desktop、Avatar、Engine、Extension 与工程生命周期平面在整体架构中处于什么位置 |
| [3. 系统上下文与信任边界](#3-系统上下文与信任边界) | 角色运行时如何面对用户、桌面、平台、模型、扩展、MCP 和本机数据 |
| [4. 运行拓扑与崩溃域](#4-运行拓扑与崩溃域) | 长期运行时由哪些进程/容器组成，失败如何隔离 |
| [5. Contracts：契约脊柱](#5-contracts契约脊柱) | 可调用服务与文档契约如何分治并保持跨语言一致 |
...
- 当前代码已经落地到什么程度：见 [Current](../../architecture/current/README.md)；
- 某个类、函数、目录如何实现：见 [Implementation](../../architecture/implementation/README.md)；
...
蓝图全部概念承诺的精确物理落点集中在 [目标物理拓扑](目标物理拓扑.md)。本页保留产品解释、设计语言与长期不变量，不复制目标目录树；若新增概念无法在该页面给出 owner、精确路径、类型、依赖、动作与完成门，该概念尚未达到可执行设计。
...
| 契约先于跨边界实现 | 跨语言、跨进程和公开 SDK 能力必须先进入 Contracts/Port，再进入生产者和消费者 |
...
| 工程生命周期是一等承重面 | Scripts、Automation、CI、Build、Release、Install 与 Ops 必须形成可验证的交付链；它服务所有运行时器官，但不成为角色器官或领域事实源 |

### 1.5 五根脊柱

五根脊柱是 Glimmer Cradle **角色运行时**的承重结构：
...
工程生命周期平面同样是平台级承重面，但它不属于这五根角色运行时脊柱，也不与认知核、中枢监督树、身体、能力生态或记忆脊柱混称。它负责把源码和 owner-local 任务可靠地变成可验证制品与宿主事务，使上述器官能够持续构建、交付、安装、恢复和演化；其长期边界见 [2.5 工程生命周期平面](#25-工程生命周期平面) 与 [ADR-0015](../../architecture/decisions/ADR-0015-工程自动化平面与交付生命周期分层.md)。
...
flowchart TB
  ...
  Engines --> Data
  Eco --> Data
  Lifecycle -.构建与交付.-> Contracts
  Lifecycle -.构建与交付.-> Kernel
  Lifecycle -.构建与交付.-> Cognition
  Lifecycle -.构建与交付.-> Avatar
  Lifecycle -.构建与交付.-> Surfaces
  Lifecycle -.构建与交付.-> Engines
  Lifecycle -.构建与交付.-> Eco
```
...
| 模块 | 生命体位置 | 拥有 | 不拥有 | 判断句 |
|---|---|---|---|---|
| Contracts | 骨架和神经接口 | 版本化 Service、文档 Schema、兼容规则与生成 DTO | 业务判断、UI view model、临时 helper | “两个边界都要理解”就先确定契约类别与 owner |
...
| Engineering Lifecycle | 平台工程生命周期平面 | owner-local 原子任务、仓库薄编排、CI 门、固定制品、安装与运维事务契约 | 角色运行时语义、领域业务逻辑、工具品牌绑定 | 它保证源码可验证地抵达宿主，不替任何器官做领域判断 |
...
长期根目录按 owner 和物理职责分层：

| 目录 | 蓝图含义 |
|---|---|
| `core/` | Kernel、Cognition、Avatar 等领域器官和平台中枢；拥有 Domain、Application、Port 与器官自身唯一进程入口 |
| `engines/` | Audio、Vision、Vector、Reasoning 等官方模型/媒体资源执行器 |
| `hosts/` | 需要与领域 owner 分离的 UnityAvatarHost、Extension Host 等具体进程承载、语言/framework 入口和崩溃域 |
| `products/` | Desktop、Personal Server 等产品装配及产品专属页面、流程和发布入口 |
| `packages/` | 公开 SDK，或至少两个真实产品 owner 稳定消费的跨产品库；两类都必须有独立 API、版本/兼容策略、测试和明确 owner |
| `contracts/` | Protobuf Service、JSON Schema 文档契约、兼容与生成配置组成的契约脊柱 |
| `native/` | C/C++ 平台原语、ABI/FFI、桌面合成与经证据下沉的性能热路径 |
| `templates/` | 开发脚手架、示例骨架与生成模板；不是运行时、产品、Host、公开包或契约事实源 |
| `configs/` | 本地配置事实源和冻结配置投影输入 |
| `assets/` | 随应用发布的只读默认资产 |
| `data/` | 本机状态、模型、安装包、缓存、工作材料、可观测性和备份 |
| `deploy/` | 部署、安装和目标环境描述 |
| `scripts/` | 工程自动化入口，不承载业务 owner |
| `docs/` | 架构、实现、参考、指南、路线图和历史证据 |

构件按“专属 owner 优先”排他归属：canonical IDL/Schema 归 `contracts/`；用户产品 Shell/组合归 `products/`；官方模型/媒体执行器归 `engines/`；平台原语、ABI/FFI 和热路径归 `native/`；领域器官、中枢及不可分离的唯一进程入口归 `core/`；只有领域 owner 在别处的可替换具体承载或不可信代码崩溃域，且没有更专属 owner 时，才归 `hosts/`；脚手架与模板归 `templates/`；最后才评估 `packages/` 准入。

因此 Audio 独立进程仍归 `engines/`，Desktop/Personal Server 入口仍归 `products/`，Cognition 唯一入口仍归 `core/cognition/host/`。当前 `templates/extension-basic/` 继续是 workspace 内的 Extension 开发模板。它不因可构建而成为公开 package；generated contract output 也不因可复用自动进入 `packages/`，其归属遵循 ADR-0013。

`products/` 只拥有产品组合。不提前创建 UI shared package。只有公开 SDK，或至少两个真实产品 owner 已经形成稳定跨产品消费，并且同时具备独立 API、版本/兼容策略、测试和明确 owner 后，候选才可进入 `packages/`；仅可独立测试/版本治理、相似视觉或未来可能复用都不够。

根级 `hosts/` 不是所有 `host/` 子目录的机械汇总。Cognition 自身唯一进程入口仍归 `core/cognition/host/`；UnityAvatarHost 需要与 Avatar Domain 分离，Extension Host 需要隔离不可信代码，因而归根级 `hosts/`。

精确当前目录事实见 Current、Implementation 和 Reference；蓝图只保留长期归属。[ADR-0014](../../architecture/decisions/ADR-0014-仓库物理分层与器官模块边界.md) 固定根职责、Host 归属和器官内部分层，[ADR-0013](../../architecture/decisions/ADR-0013-契约脊柱与跨进程服务架构.md) 固定 Contract Spine 与跨进程 Service。

上述完成态已由 M12 candidate 落到物理仓库：`contracts/` 是唯一 Contract Spine，`protocol/` 已删除；Avatar 分为 `core/avatar/` 与 `hosts/unity-avatar-host/`，Extension Host 执行容器位于 `hosts/extension-host/`。当前细节见 [Contract Spine Reference](../../reference/protocol.md) 和 [M12](../../roadmap/milestones/M12-契约脊柱与跨进程服务架构重建.md)。

### 2.5 工程生命周期平面

Scripts、Automation、CI、Build、Release、Install 与 Ops 共同构成一条平台级工程生命周期平面。它与运行时架构同等需要长期 owner、边界、契约、失败语义和演化能力；不得作为功能完成后的后补工具、无承诺 backlog 或散落命令集合。它服务 Contracts、Kernel、Cognition、Desktop、Avatar、Engine、Extension、Data 等所有运行时器官，但不拥有这些器官的领域语义，也不进入角色运行时监督树。
...
| 层 | 唯一职责 | 禁止 |
|---|---|---|
| Source | 保存源码、契约、配置、资产与任务定义的权威输入 | 携带机器私有状态或隐式生成物 |
| Owner-local atomic task | 由 package/product/host/domain owner 表达本领域构建、校验、准备、打包原子语义 | 把领域逻辑上提到 root 或 workflow |
| Repository thin orchestration | 只编排真正跨 owner 的公共任务与依赖顺序 | 复制 package-local 实现、制造第二套 prehook |
| CI | 在干净环境调用与本地相同的公共任务并保存结构化证据 | 在 workflow 内重写业务逻辑或只靠 tag Release 代替 PR 门 |
| Fixed artifact | 以 digest、manifest、provenance/attestation 固定可交付输入 | Release 时重新解释漂移源码或复用未固定产物 |
| Install / Ops | 由宿主级事务 owner 串行化安装、更新、备份、恢复、回滚和回收 | 多个入口并发写同一宿主、让被替换进程自我终止事务 |

每个任务，无论由哪种语言或工具实现，都必须能够声明并验证：
...
这些声明是任务的工程契约，不要求绑定某个特定 task runner。工具可以替换，任务语义、owner、fixed state 和验证证据不能随工具替换而漂移。

工程生命周期平面的长期不变量是：

- **local/CI parity**：本地和 CI 调用同一公共任务，环境差异只通过显式输入表达；
- **可复现**：相同 fixed source 与固定依赖产生可验证等价输出；
- **固定制品**：Release 只消费已经固定并验证的 artifact，不在发布阶段重构建漂移输入；
- **事务恢复**：安装与运维动作具备宿主级锁、幂等、回滚、备份域隔离和失败终态；
- **最小权限**：状态查询、日志读取与 dry-run 不提前创建 token、环境、状态或权限副作用；
- **跨平台 core + thin wrapper**：核心任务语义跨平台，PowerShell/Bash 等 wrapper 只处理平台接入；
- **结构化诊断**：失败由稳定 exit code、阶段、owner、摘要和恢复入口解释，不靠自由文本猜测；
- **删除旧入口**：新链通过验收后删除重复 prehook、旧 root 实现、workflow 私有逻辑和无期限兼容 wrapper。

当前 Scripts、workflow、构建、发布、安装和运维实现仍以 Current、Implementation、Reference 与 Guide 为准；接受本蓝图不表示这些目标已经落地。落地承诺、风险矩阵与删除门见 [M13：工程自动化脊柱与交付生命周期闭环](../../roadmap/milestones/M13-工程自动化脊柱与交付生命周期闭环.md)。
...
flowchart TB
  ...
  Gateway[Kernel Surface Gateway<br/>Connect target]
  Kernel[Kernel / Stage Manager]
  Cognition[Python Cognition]
  Audio[Audio Engine]
  Avatar[UnityAvatarHost]
  ...
  Desktop <--> Gateway
  Gateway --> Kernel
  Kernel <-->|gRPC target| Cognition
  Kernel <-->|gRPC target| Audio
  Kernel <-->|gRPC target| Avatar
  Avatar <--> Native
  Kernel <-->|gRPC target| Ext
  Kernel <--> MCP
  Kernel <--> UserSkill
  Kernel --> Data
  Cognition --> Data
...
长期结构是一族受监督进程与进程内模块化器官，而不是把所有功能堆进一个进程，也不是把每个领域模块拆成微服务。Kernel 是单进程模块化中枢；Cognition 是具有单一心智主权和一致性边界的独立进程模块化单体；具体 Unity Avatar Host 与 Extension Host 是 `hosts/` 下的独立崩溃域。每个 runtime 必须说明：
...
| Extension Host / Worker | 对应扩展能力失败，撤销 catalog/Port/handler；Kernel 继续监督或重启 | 第三方代码拖垮 Kernel 或留下旧 handler |
...
长期目标不是让每个器官互相发现，而是由 Kernel 维持一棵受监督的星形服务拓扑：

```text
Web / Desktop Surface
  <-> Kernel Surface Gateway (Connect target)
  <-> Kernel Application / Projection / Policy
      <-> Cognition Service (gRPC target)
      <-> Engine Services (gRPC target)
      <-> Avatar Service (gRPC target)
      <-> Extension Host Service (gRPC target)
```

Kernel 继续拥有动态端点 catalog、进程世代、权限、路由、readiness、Projection 和回收。器官不横向自由调用；跨器官协作回到 Kernel。Surface Gateway 是 BFF/Projection Adapter，只暴露面向产品表面的 Query、Command 与 Stream，不把内部 Service 或领域对象交给浏览器。

这套拓扑与 [ADR-0009](../../architecture/decisions/ADR-0009-本地监督树与动态端点治理.md) 组合：gRPC/Connect 固定契约和调用语义，ADR-0009 固定本地监督、动态回环端点和真实 readiness。二者都不要求固定端口。

## 5. Contracts：契约脊柱

Contracts 是摇篮多语言、多进程、多 Surface 的稳定语义脊柱。它采用 IDL-first，但不强迫所有结构使用同一种 IDL。

### 5.1 契约分类与 owner

| 类别 | 语义 | 权威形式 |
|---|---|---|
| Command | 请求 owner 执行有副作用动作 | 版本化 Protobuf Service method |
| Query | 读取 owner 的事实或 Projection | 版本化 Protobuf Service method |
| Event | 已发生事实的不可变通知 | typed Protobuf event 与明确投递语义 |
| Stream | 有序、可取消、需要流控的数据序列 | Protobuf streaming service；Surface 由 Connect 能力验证 |
| Document | 可独立存储、编辑和校验的配置/package 文档 | JSON Schema |

配置、Character Package、Extension manifest/package 和动态 Skill/tool 参数继续由 JSON Schema 拥有。跨进程可调用能力由 Protobuf Service 拥有。同一消息不得由 Protobuf 与 JSON Schema 各手写一份。

### 5.2 目标物理形态与版本

`contracts/proto/glimmer/<owner>/v1/` 按服务 owner 和兼容世代组织 Protobuf；`contracts/json-schema/{config,character,extension,skill}/` 按文档 owner 组织 JSON Schema。应用 SemVer 与 `v1/v2` 契约世代分离；兼容演进留在同一世代，无法兼容的语义才引入新世代。

生成物只读且可复现。Kernel、Cognition、Avatar、Engine 等领域模型仍归各自 owner；generated DTO、stub、transport context 只存在于边界 Adapter，不进入 Domain/Application/Port。

### 5.3 调用、错误与因果

每个方法必须定义方向、deadline、cancellation、稳定错误 code、typed detail、trace metadata、causation/correlation、进程世代、readiness 前置和幂等/补偿语义。成功必须说明承诺已经成立；异步 accepted/pending 必须可查询。required service 未 ready 时阻止对应 Ingress；optional service 失败时撤销能力并投影 degraded。

gRPC status 负责通用调用结果，领域失败由 typed detail 表达；调用方不得解析自由文本。deadline 到期或取消后，服务端和上游调用停止无用工作。trace 解释机器链路，causation 解释角色生命因果，二者关联但不合并。

### 5.4 Control plane 与 data plane

普通 RPC 只承载控制、小型结构化结果和 typed reference。大型音频、图像、模型、Blob 与高频 Avatar 数据使用有界 stream、受管 Blob lease/reference，或经平台验证的 shared memory/专用通道。reference 必须携带 owner、media type、size、digest、expiry 和访问范围；数据面失败、过期、取消与清理必须可见。

Protobuf 官方定位适合有限大小的结构化消息，不是大型媒体容器；因此不能把无界 base64 或模型塞进普通消息。

### 5.5 兼容、供应链与可替换实现

- Protobuf 使用 Buf lint/breaking；JSON Schema 使用 dialect、fixture 和项目 compatibility 门；
- TypeScript、Python、C# 必须通过 serialization round-trip 和 Adapter contract test；
- canonical `.proto`/JSON Schema 全部在仓库，生成器和插件版本固定且可离线复现；
- Buf Schema Registry、远程插件和其他托管服务不是必需依赖；
- gRPC 与 Connect 是 Adapter/Transport 默认实现，不是领域边界；替换实现不应修改领域模型或业务 Port；
- 破坏性迁移必须有 producer/consumer 顺序、观察指标、删除条件和回退边界；
- 不保留无退出条件双轨。

DDD/Hexagonal、BFF/Projection、Saga 和 control/data plane 是架构模式；Protobuf、gRPC、JSON Schema、Buf 与 Connect 是 IDL、规范、工具或 transport 生态。摇篮组合这些成熟方法以适配 TS/Python/C#、本地多进程、多个 Surface、Extension 与媒体器官，不复制任何厂商模板。技术治理、许可与防锁定规则由 [ADR-0013](../../architecture/decisions/ADR-0013-契约脊柱与跨进程服务架构.md) 固定。
...
- Ingress Gate；
- runtime readiness；
- Extension Host 的监督、权限、catalog、调用编排与 Projection；
- Skill Plane；
- capability registry；
- 状态投影；
- logs/trace/DLQ 汇聚；
...
Kernel 进程不得执行第三方 Extension handler；真正执行不可信扩展代码的 Extension Host 是 `hosts/` 下的独立进程和崩溃域。
...
Kernel 保持单进程模块化中枢，长期物理层次是 `domain/`、`application/`、`ports/`、`adapters/`、`runtime/`、`composition/`：

```text
domain <- application -> ports <- adapters
                 runtime -> application/ports
composition -> domain/application/adapters/runtime
```

- Domain 只拥有监督策略、状态机和不变量，不执行 OS/process 副作用；
- Application 以有名称的启动、停止、重启、恢复 use case 协调 Domain 与 Ports；
- Ports 只表达 process、clock、config、filesystem、transport、observability 等所需或对外能力；
- Adapters 实现 OS process、filesystem、transport、persistence、observability 和 contract 边界；
- Runtime 执行生命周期、进程世代、readiness 采样、受管运行与资源回收，不另写监督策略；
- Composition 是创建具体实现和接线的唯一根。

依赖方向固定为 `domain <- application -> ports <- adapters`。Runtime 可以调用 Application use case 并使用 Ports；Domain/Application 不 import Runtime/Adapters，Ports 不 import 具体实现，只有 Composition 看见所有层。Projection 只有 Application 中有名称的 projection mapper/read model owner 能从 Domain facts 与 Runtime facts 生成；Runtime 通过 Application 输入边界提交 facts，Domain 和 Runtime 都不直接写 Projection。

当前 `foundation/`、`application/services/`、`lifecycle/`、`infrastructure/`、`host/` 等物理事实只由 Current/Implementation 描述。M12 必须证明副作用、import 方向和 Projection 单写入 owner 已收口，并在配置、测试、构建与打包消费者归零后删除旧聚合目录和兼容导出；不能只把旧目录换名。
...
Cognition 是一个独立受监督进程内的模块化单体，不是一组微服务。`identity`、`persona`、`affect`、`experience`、`memory`、`conversation`、`context`、`deliberation`、`volition` 都属于同一心智主权和一致性边界；领域目录的区分不产生网络端点或独立数据库主权。

长期内部物理层次是 `domain/`、`application/`、`ports/`、`adapters/`、`host/`。内部领域模块通过有名称的 Application use case、模块 API、明确本地事务边界和领域事件协作；Ports 只表达 Cognition 边界所需或提供的外部能力，例如 provider、persistence、clock 与 Kernel contract。不得为每个内部模块制造 Port 或进程内伪远程服务。provider、transport、persistence、observability 才是 Adapter；generated contract DTO/stub 只能停留在 contract Adapter 边缘，不进入心智 Domain/Application。

同一 Cognition 进程内不使用 RPC、共享 generated DTO 或万能 Event Bus 模拟分布式系统。只有流程跨越 Kernel、Cognition、Avatar、Engine 或 Extension Host 等进程与一致性边界时，才由明确 owner 使用 Saga/Process Manager，并定义幂等、补偿、deadline/cancellation、causation/correlation、重试与失败终态；不使用跨器官分布式数据库事务。

旧平级领域/技术目录和 `protocol/generated/` 已在 M12 迁移 imports、装配、生成、测试与打包消费者后删除；当前 Cognition 物理层次以 Current/Implementation 为准。
...
该领域长期物理归属是 `core/avatar/`：一个可在 Unity 外独立 `dotnet build` / `dotnet test` 的 SDK-style C# class library/source package，拥有 Domain、Application 与 Ports。Core Avatar 源码和编译目标不得引用 `UnityEngine`、Cubism、Unity asmdef、gRPC/Connect/Protobuf runtime、generated DTO 或具体 transport。

UnityAvatarHost 是 `hosts/` 下的 Unity Project 和具体进程承载，只能单向消费仓库内固定工具链本地构建并校验的唯一 Core Avatar assembly/artifact。Unity/Cubism driver、framework 生命周期和 contract DTO 映射归 Host Adapter；构建/打包先生成 Core assembly，再供 Unity Host 引用。不得复制或投影第二份 Avatar Domain 源码，也不得把外部 NuGet/云 registry 作为唯一交付来源。

这是 [ADR-0014](../../architecture/decisions/ADR-0014-仓库物理分层与器官模块边界.md) 对 [ADR-0005](../../architecture/decisions/ADR-0005-Character-Avatar-Surface-Host分层.md) 和 [ADR-0007](../../architecture/decisions/ADR-0007-UnityAvatarHost程序集边界与SDK投影.md) 相关物理/编译落点的部分替代；二者的语义分层与单向依赖继续有效。M12 Slice 5 candidate 已完成独立 Core 构建测试、Host Adapter 映射、单向 assembly reference 和旧六 asmdef/旧目录/复制源码删除门；后续不得退回混合 Unity Domain。
...
连接存在不能等于 Avatar ready。静态贴图预览不能冒充正式 Live2D/Unity ready。具体 gate 和命名由 [ADR-0005](../../architecture/decisions/ADR-0005-Character-Avatar-Surface-Host分层.md) 固定。
...
执行第三方代码的 Extension Host 必须物理位于 `hosts/`，作为 Kernel 监督的独立进程和崩溃域。Kernel 保留监督、权限、catalog、调用编排和 Projection，不在自身进程内加载 Extension handler；Host 崩溃或停用时必须撤销 handler、catalog、lease、timer、订阅与子进程。具体 leaf 由实现切片按 package/进程身份和命名规范确定，不能用含混 `worker`/`runtime` 名掩盖 owner。
...
```text
Composition / bootstrap adapters
  -> Transport
  -> Application Graph
...
Close Ingress
  ...
  -> stop transports
  -> flush owner adapters and stores
```

停机必须释放订阅、计时器、stream、transport、受管进程、lease、临时文件、日志和未刷新的持久化写入。所有收尾都有明确期限；不得在停机路径运行长期记忆巩固、模型推理或其他可延后任务。期限结束后监督者可直接回收进程树，持久待办由下次启动或其他维护 Worker 接续。
...
| 可交付 | owner-local 原子任务经薄编排与同构 CI 形成固定制品，再由宿主级事务安全安装、恢复和运维 |

## 16. 架构审查清单

任何跨层设计进入主线前必须回答：

1. 这个能力属于 Cognition、Kernel、Desktop、Avatar、Engine、Host、Package、Native、Extension、Skill Plane、Contracts 还是 Data？
2. 它是器官、能力、表面、投影还是缓存？
3. 谁是唯一事实源？
4. 哪些结构跨语言、跨进程、跨信任边界？
5. 是否需要 Protobuf Service、JSON Schema Document 或公开 Port？
...
17. 它的物理位置是否与领域 owner、进程崩溃域和产品装配一致？旧路径是否有可验证删除条件？
18. 它的 owner-local 原子任务是否声明 inputs、outputs、side effects、lock、idempotency、confirm、dry-run、exit code、diagnostic 和 recovery？
19. root 与 workflow 是否只调用公共任务，Release 是否只消费 fixed artifact，Install/Ops 是否由唯一宿主事务 owner 串行化？
20. 本地与 CI 是否同构，跨平台差异是否由 thin wrapper 隔离，旧自动化入口是否有删除门？
...
| Contracts / Contract Spine | 跨语言、跨进程和公开 SDK 的契约脊柱；以 Protobuf Service 与 JSON Schema 文档契约分治 |
| Surface Gateway | Kernel 面向 Web/Desktop Surface 的专用 BFF/Projection 服务边界 |
...
| Engineering Lifecycle / 工程生命周期平面 | Scripts、Automation、CI、Build、Release、Install 与 Ops 组成的平台级交付承重面；服务所有运行时器官，但不是角色运行时器官 |
| Atomic Task | 由 package、product、host 或领域 owner 持有的最小可独立声明、执行和验证的工程任务 |
| Fixed Artifact | 从固定源码与依赖生成、带 manifest/digest/provenance 证据且供 Release/Install/Ops 消费的不可漂移制品 |
| Host-global Transaction Owner | 在单一宿主上串行化安装、更新、备份、恢复、回滚与回收的唯一事务 owner |
