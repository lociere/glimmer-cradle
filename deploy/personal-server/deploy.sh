...
prepare_environment() {
  ...
  if ! grep -q '^GLIMMER_CRADLE_IMAGE=' "$DEPLOYMENT_ENV_FILE"; then
    set_env_value "$DEPLOYMENT_ENV_FILE" GLIMMER_CRADLE_IMAGE "${IMAGE_REPOSITORY}:0.1.2-rc.1"
  fi
...
next_candidate_image() {
  if [[ -n "${GLIMMER_CRADLE_CANDIDATE_IMAGE:-}" ]]; then
    ...
    read_env GLIMMER_CRADLE_IMAGE "${IMAGE_REPOSITORY}:0.1.2-rc.1"
  fi
}
...
candidate_image() {
  ...
  printf '%s:%s-%s%s-%s' "$IMAGE_REPOSITORY" "${version:-0.1.2-rc.1}" "$revision" "$dirty" "$timestamp"
}
