...
int main() {
  ...
  for (;;) { DWORD size{}; if (!std::cin.read(reinterpret_cast<char*>(&size),4) || size > 1024*1024) break; std::vector<char> body(size); if (!std::cin.read(body.data(),size)) break; size_t p=0; DWORD op{}, argc{}; std::string session, program, cwd; if (!read32(body,p,op) || !readString(body,p,session)) { reply(2,"",0,0); continue; }
    ...
    else if (op == 2) { TerminateJobObject(job,1); JOBOBJECT_BASIC_ACCOUNTING_INFORMATION info{}; QueryInformationJobObject(job,JobObjectBasicAccountingInformation,&info,sizeof(info),nullptr); const DWORD active=info.ActiveProcesses; if (active==0) { CloseHandle(job); job=nullptr; reply(0,session,0,0); break; } reply(3,session,0,active); }
