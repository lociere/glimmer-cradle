...
export function ExtensionInstall({ snapshot, controller }: { snapshot: ExtensionsSnapshot; controller: ExtensionsController }): JSX.Element {
  ...
  return <section className={styles.install} aria-label="安装扩展" data-role="extension-install-section">
    ...
    <div className={styles.form} data-role="extension-install-form">
      <SelectField label="来源类型" dataField="source-kind" disabled={locked} value={source} options={[{ value: 'repository', label: '仓库 Release' }, { value: 'file', label: '本地 .gcex' }, { value: 'registry', label: 'Registry 条目' }, { value: 'release_manifest', label: 'Release Manifest' }]} onChange={(value) => { setSource(value); setValidation(''); }} />
      {source === 'repository' && <>{input('repository', '仓库', 'repository', 'publisher/repo')}{input('tag', 'Tag', 'tag', 'v1.2.3')}</>}
      {source === 'registry' && <>{input('catalog', 'Catalog URL', 'catalog-url')}{input('id', 'Extension ID', 'extension-id')}<SelectField label="Channel" dataField="channel" disabled={locked} value={fields.channel} options={['stable', 'beta', 'nightly'].map((value) => ({ value, label: value }))} onChange={(channel) => setFields({ ...fields, channel })} /></>}
