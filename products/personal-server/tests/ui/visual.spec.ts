...
async function expectDocumentLocked(page: import('@playwright/test').Page) {
...
async function setTheme(page: import('@playwright/test').Page, theme: '深色' | '浅色') {
  await page.getByRole('button', { name: '外观设置' }).click();
  await page.getByRole('group', { name: '主题' }).getByRole('button', { name: theme, exact: true }).click();
  await page.keyboard.press('Escape');
}

test('captures the compact anonymous login surface', async ({ page }) => {
  ...
  try {
    ...
    await expect(page.locator('[data-role="login-form"]')).toBeVisible();
    await page.mouse.move(0, 0);
    await expect(page).toHaveScreenshot('login-dark.png', { animations: 'disabled', fullPage: true });
...
test('captures settings provider, enhancement and confirmation layouts', async ({ page }, testInfo) => {
  ...
  const fixture = await startPersonalServerUiFixture();
  try {
    await page.goto(`${fixture.baseUrl}/config`); await page.getByLabel('访问令牌').fill('server-secret'); await page.getByRole('button', { name: '连接 Personal Server' }).click();
    ...
    if (testInfo.project.name === 'personal-server-narrow') {
      await page.getByRole('button', { name: '选择配置分类', exact: true }).click();
    ...
    await page.getByRole('button', { name: '外观设置' }).click();
    await page.mouse.move(0, 0); await expect(page).toHaveScreenshot('appearance-popover-dark.png', { animations: 'disabled' });
    await page.keyboard.press('Escape');
    await selectSettingsSection(page, '记忆与检索');
    ...
    await page.keyboard.press('Escape');
    if (testInfo.project.name === 'personal-server-desktop') { await setTheme(page, '浅色'); await selectSettingsSection(page, 'AI 与路由'); await page.mouse.move(0, 0); await expect(page).toHaveScreenshot('settings-models-light.png', { fullPage: true, animations: 'disabled' }); }
  } finally { await fixture.stop(); }
});
...
test('captures activity list and details in dark, light and narrow layouts', async ({ page }, testInfo) => {
  ...
  try {
    ...
    if (testInfo.project.name === 'personal-server-desktop') {
      await page.getByLabel('级别', { exact: true }).click();
      await expect(page.getByRole('listbox', { name: '级别' })).toBeVisible();
      await expect(page).toHaveScreenshot('activity-level-popover-dark.png', { animations: 'disabled' });
      await page.keyboard.press('Escape');
    }
    await page.getByRole('button', { name: /^查看事件/ }).first().click(); await expect(page.getByRole('dialog', { name: '事件详情' })).toBeVisible();
    await page.mouse.move(0, 0); await expect(page).toHaveScreenshot('activity-details.png', { animations: 'disabled', fullPage: true });
    await page.keyboard.press('Escape');
    if (testInfo.project.name === 'personal-server-desktop') {
      await setTheme(page, '浅色'); await page.mouse.move(0, 0);
...
test('captures extensions directory, installation and details in dark, light and narrow layouts', async ({ page }, testInfo) => {
  ...
  try {
    ...
    await trigger.click(); await expect(page.locator('[data-role="extension-detail"]')).toBeVisible();
...
test('captures capability and system maintenance workspaces', async ({ page }) => {
  await page.addInitScript(() => window.localStorage.setItem('personal-server-theme', 'dark'));
  const fixture = await startPersonalServerUiFixture({ zeroProvider: true });
  try {
    await page.goto(`${fixture.baseUrl}/capabilities`);
    await page.getByLabel('访问令牌').fill('server-secret');
    ...
    await expectDocumentLocked(page);
    await page.mouse.move(0, 0);
    await expect(page).toHaveScreenshot('capability-catalog-dark.png', { animations: 'disabled', fullPage: true });
    await page.goto(`${fixture.baseUrl}/system/operations`);
    await expect(page.locator('[data-role="view-system-settings"]')).toBeVisible();
    await page.mouse.move(0, 0);
    await expect(page).toHaveScreenshot('system-maintenance-dark.png', { animations: 'disabled', fullPage: true });
  } finally { await fixture.stop(); }
});
...
test('captures restored and empty conversation in both themes and narrow layout', async ({ page }, testInfo) => {
  ...
  const fixture = await startPersonalServerUiFixture();
  try {
    await page.goto(`${fixture.baseUrl}/data`);
    ...
    await page.locator('[data-role="message-input"]').focus();
    await expect(page).toHaveScreenshot('conversation-composer-focus-dark.png', { animations: 'disabled', fullPage: true });
    if (testInfo.project.name === 'personal-server-desktop') {
      await setTheme(page, '浅色');
      await page.mouse.move(0, 0);
  ...
  try {
    await page.goto(`${empty.baseUrl}/data`);
    await page.locator('#access-token').fill('server-secret');
    await page.getByRole('button', { name: '连接 Personal Server' }).click();
...
test('captures chromeless navigation, row hover and focus-visible interaction states', async ({ page }, testInfo) => {
  await page.addInitScript(() => window.localStorage.setItem('personal-server-theme', 'dark'));
  const fixture = await startPersonalServerUiFixture();
  try {
    ...
    await expect(page.locator('[data-role="status-line"]')).toContainText('正在观察日志流');
    if (testInfo.project.name === 'personal-server-desktop') {
      await page.locator('[aria-label="全局导航"] [data-route="system"]').hover();
      await expect(page).toHaveScreenshot('interaction-navigation-hover.png', { animations: 'disabled' });
      await page.getByRole('button', { name: /^查看事件/ }).first().hover();
      await expect(page).toHaveScreenshot('interaction-log-row-hover.png', { animations: 'disabled' });
    } else {
      await page.getByRole('button', { name: /^查看事件/ }).first().focus();
      await expect(page).toHaveScreenshot('interaction-log-row-focus.png', { animations: 'disabled' });
    }
  } finally { await fixture.stop(); }
});
