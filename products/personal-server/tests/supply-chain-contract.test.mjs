...
test('Release job 只下载并发布 build job 固定的制品', () => {
  ...
  assert.doesNotMatch(releaseUses, /docker|package-release|pnpm build/);
});

test('Personal Server fixed artifact 独立验证 manifest、SBOM、provenance 与 attestation', async () => {
  ...
  try {
    ...
    await execNode('verify-release.mjs', [output]);
    await writeFile(artifact, Buffer.from('tampered release'));
    await assert.rejects(() => execNode('verify-release.mjs', [output]));
  } finally {
