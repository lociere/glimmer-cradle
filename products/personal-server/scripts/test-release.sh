...
TEST_ROOT="$(mktemp -d)"
HOST_ROOT="/tmp/glimmer-cradle-release-test.$$"
VERSION=9.8.7
...
RUN_ROOT="${HOST_ROOT}/run"
CONFIG_ROOT="${TEST_ROOT}/config"
CLI_PATH="${TEST_ROOT}/bin/glimmer-cradle"

cleanup() {
  if (( EUID == 0 )); then
    rm -rf -- "$TEST_ROOT" "$HOST_ROOT"
  else
    sudo rm -rf -- "$TEST_ROOT" "$HOST_ROOT"
  fi
}
...
as_root install -d -o 0 -g 0 -m 0755 "$HOST_ROOT"
STATE_ROOT="${HOST_ROOT}/state"
export GLIMMER_CRADLE_DOCKER_SOCKET_PATH="${HOST_ROOT}/docker.sock"
...
grep -q -- 'DOCKER_SOCKET_PATH="${GLIMMER_CRADLE_DOCKER_SOCKET_PATH:-/var/run/docker.sock}"' "$PACKAGED_DEPLOY"
grep -q -- 'src="$DOCKER_SOCKET_PATH",dst=/var/run/docker.sock,readonly' "$PACKAGED_DEPLOY"
[[ -f "${TEST_ROOT}/light/glimmer-cradle-personal-server/lib/host-transaction.sh" ]]
...
run_installer() {
  ...
  local -a command=(env
    ...
    GLIMMER_CRADLE_DOCKER_SOCKET_PATH="${HOST_ROOT}/docker.sock"
    "$@"
...
run_light_installer() {
  ...
  local light_state_root="${HOST_ROOT}/light-state"
  local light_config_root="${TEST_ROOT}/light-config"
  local light_cli_path="${TEST_ROOT}/light-bin/glimmer-cradle"
  local fake_bin="${REPO_ROOT}/deploy/personal-server/tests/fixtures/install-interrupt"
...
EOF
  ...
  local -a command=(env
    ...
    GLIMMER_CRADLE_RUN_ROOT="${HOST_ROOT}/light-run"
...
run_full_installer() {
  ...
  local full_state_root="${HOST_ROOT}/full-state"
  ...
  local -a command=(env
    ...
    GLIMMER_CRADLE_RUN_ROOT="${HOST_ROOT}/full-run"
...
as_root mkdir -p "$INSTALL_ROOT" "$CONFIG_ROOT" "$(dirname -- "$CLI_PATH")"
...
if run_installer "$OUTPUT_ROOT" \
  PATH="${REPO_ROOT}/deploy/personal-server/tests/fixtures/install-interrupt:${PATH}" \
  GLIMMER_CRADLE_DOCKER_BIN="${REPO_ROOT}/deploy/personal-server/tests/fixtures/install-interrupt/docker" \
...
for deploy_exit in 70 75 78 130 143; do
  deploy_failure="${TEST_ROOT}/deploy-failure-${deploy_exit}"
  deploy_failure_stage="${TEST_ROOT}/deploy-failure-stage-${deploy_exit}"
  mkdir "$deploy_failure" "$deploy_failure_stage"
  cp -a "${TEST_ROOT}/full/glimmer-cradle-personal-server" "$deploy_failure_stage/"
  cat > "${deploy_failure_stage}/glimmer-cradle-personal-server/deploy.sh" <<EOF
#!/usr/bin/env bash
set -Eeuo pipefail
echo "注入 deploy 故障 ${deploy_exit}" >&2
exit ${deploy_exit}
EOF
  chmod 0755 "${deploy_failure_stage}/glimmer-cradle-personal-server/deploy.sh"
  (
    cd "${deploy_failure_stage}/glimmer-cradle-personal-server"
    deploy_digest="$(sha256sum ./deploy.sh | cut -d' ' -f1)"
    sed -i -E "s#^[0-9a-f]{64}  \\./deploy\\.sh\$#${deploy_digest}  ./deploy.sh#" RELEASE-MANIFEST.sha256
  )
  tar -C "$deploy_failure_stage" -czf "${deploy_failure}/${FULL}" glimmer-cradle-personal-server
  cp "${OUTPUT_ROOT}/glimmer-cradle-installer.sh" "$deploy_failure/"
  (
    cd "$deploy_failure"
    sha256sum "$FULL" glimmer-cradle-installer.sh > SHA256SUMS
  )
  set +e
  run_installer "$deploy_failure" \
    GLIMMER_CRADLE_DOCKER_BIN="${REPO_ROOT}/deploy/personal-server/tests/fixtures/install-interrupt/docker" \
    GLIMMER_CRADLE_TEST_DOCKER_MODE=success \
    GLIMMER_CRADLE_TEST_IMAGE_ID="$IMAGE_ID" \
    >"${TEST_ROOT}/deploy-failure-${deploy_exit}.log" 2>&1
  observed_exit=$?
  set -e
  [[ "$observed_exit" == "$deploy_exit" ]] || {
    echo "安装器吞掉 deploy 退出码 ${deploy_exit}，实际 ${observed_exit}。" >&2
    sed -n '1,160p' "${TEST_ROOT}/deploy-failure-${deploy_exit}.log" >&2
    exit 1
  }
  [[ "$(readlink -f "${INSTALL_ROOT}/current")" == "$PREVIOUS_RELEASE" ]]
  as_root cmp "${TEST_ROOT}/deployment.env.baseline" "${CONFIG_ROOT}/deployment.env"
